 ...
 * No external dependencies — uses a built-in extension-to-category map.
 */
...
/**
 * Extension-to-category map. Extensions are stored without the leading dot
 * and in lowercase for efficient lookup.
 */
const EXTENSION_TO_CATEGORY = new Map<string, FileCategory>([
  ...
  ["3gp", "video"],
  ["ogv", "video"],
  ["ts", "video"], // Note: .ts is also TypeScript — video wins here for media context
 ...
 * Accepts either a full path (e.g. "/foo/bar.jpg") or a bare extension
 * (e.g. ".jpg" or "jpg").
 */
