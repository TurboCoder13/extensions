...
describe("transliterate", () => {
  ...
  describe("special symbols", () => {
    ...
    it("converts em/en dashes to regular dashes", () => {
      expect(transliterate("word\u2014word")).toBe("word-word"); // em-dash → single dash
      expect(transliterate("word\u2013word")).toBe("word-word"); // en-dash → single dash
    });
  ...
  describe("CJK handling", () => {
    it("preserves CJK characters (no romanization in native implementation)", () => {
    ...
    it("removes CJK characters with removeUnmapped option", () => {
      expect(transliterate("\u65e5\u672c\u8a9e", { removeUnmapped: true })).toBe("");
    });

    it("preserves ASCII alongside CJK with removeUnmapped", () => {
