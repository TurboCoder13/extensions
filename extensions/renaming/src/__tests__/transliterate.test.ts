...
describe("transliterate", () => {
  describe("accented characters", () => {
    ...
    it("converts \u00c5ngstr\u00f6m to Angstroem", () => {
      expect(transliterate("\u00c5ngstr\u00f6m")).toBe("Angstroem");
    });

    it("converts M\u00fcller to Mueller", () => {
  ...
  describe("German characters", () => {
    ...
    it("converts \u00f6 to oe", () => {
      expect(transliterate("sch\u00f6n")).toBe("schoen");
    });
  });
  ...
  describe("special symbols", () => {
    ...
    it("converts em/en dashes to regular dashes", () => {
      expect(transliterate("word\u2014word")).toBe("word-word"); // em-dash
  ...
  describe("CJK passthrough", () => {
    it("passes through CJK characters (no romanization)", () => {
      // Built-in transliteration does not include CJK romanization
      expect(transliterate("\u65e5\u672c\u8a9e")).toBe("\u65e5\u672c\u8a9e");
    });

    it("preserves CJK mixed with ASCII", () => {
      expect(transliterate("\u65e5\u672c\u8a9etest")).toBe("\u65e5\u672c\u8a9etest");
    });

    it("removes CJK characters when removeUnmapped is true", () => {
      expect(transliterate("\u65e5\u672c\u8a9e", { removeUnmapped: true })).toBe("");
      expect(transliterate("Hello \u4e16\u754c", { removeUnmapped: true })).toBe("Hello ");
    });
  });
